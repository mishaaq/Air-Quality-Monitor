/*
  ...
  Added support for SH1106 128x64 display by michap (9/2018).
  Works with buffer only - take 1024byte RAM !!!
...
void OLED::display() {
    for (int index = 0; index < 8; index++) {
        _sendTWIcommand(SH1106_SETSTARTPAGE + index);
        _sendTWIcommand(SH1106_SETLOWCOLUMN + 2);
        _sendTWIcommand(SH1106_SETHIGHCOLUMN); 
		int pixel=0;
		while(pixel<128){	
		Wire.beginTransmission(SSD1306_ADDR);
		Wire.write(SSD1306_DATA_CONTINUE);
		for (uint8_t x=0; x<16; x++) {
			Wire.write(buffer[index * 128 + pixel]);
			pixel++;
		}
		Wire.endTransmission();
        }
...
void OLED::writeData(char * pdata) {
	...
	while(cnth < pdata[1]){
		while(cntw < pdata[0]) {
			buffer[(cy+cnth)*128+cx+cntw]=pdata[pdatac];
			pdatac++;
			cntw++;
		}
		cntw =0;
...
void  OLED::fillRect(byte w, byte h, byte fill)
{
	...
	while(cnth < h){
		while(cntw < w) {
			buffer[(cy+cnth)*128+cx+cntw]=fill;
			g_oled_count++;
			cntw++;
		}
		cntw =0;
...
void OLED::write(uint8_t value) {
	...
	while(cnth < symbol_h){
		while(cntw < symbol_w) {
			buffer[(cy+cnth)*128+cx+cntw]=curr_font[g_oled_count];
			g_oled_count++;
			cntw++;
		}
		cntw =0;
